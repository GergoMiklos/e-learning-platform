...
export default function ParentPageComp({
  user,
  addFollowedCode,
  onAddFollowedCodeChange,
  isAddFollowedDisabled,
  ...
  return (
    <div className="container">
      ...
      {!user.followedStudents?.length
        ? 'No Students'
        : user.followedStudents.map((student) => (
            <ParentElementCont key={student.id} student={student} />
          ))}
    </div>
  );
}
