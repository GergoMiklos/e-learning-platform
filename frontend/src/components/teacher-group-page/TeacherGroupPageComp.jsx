...
export default function TeacherGroupPageComp({
  group,
  editPath,
  onNavigateBack,
}) {
    ...
    <div className="container">
      ...
      {group.news && (
        <section className="row alert alert-warning my-3">
          ...
          <div className="col-12 ">{group.news}</div>
        </section>
      )}
      ...
      <section className="row rounded shadow bg-light my-3 p-3 justify-content-between">
        ...
        <Link to={`${match.url}/new`}>
          <button className="btn btn-lg btn-primary">New</button>
        </Link>
      </section>

      <Route
        path={`${match.url}/new`}
        render={(props) => <NewTestDialogCont groupId={group.id} {...props} />}
      />
