    ...
    return (
        <div className="container bg-secondary">
            ...
            <section className="my-3 table-responsive-sm">
                {!studentStatus.studentTaskStatuses?.length ? "No tasks" :
                    <table className="table table-striped bg-light">
                        ...
                        {studentStatus.studentTaskStatuses.map(status =>
                            <tr key={status.id}>
                                <td
                                    className={studentStatus.currentTestTask?.id === status.testTask?.id ?
                                        'font-weight-bold' : ''}
                                >
                                ...
                                <td className="justify-content-center d-flex text-center">
                                    <div className={(status.wrongSolutionsInRow > 0 ?
                                        `bg-warning rounded-circle mt-1 p-${Math.min(status.wrongSolutionsInRow, 2)}` :
