...
const mockJoinGroupCodeValue = mockJoinGroupCodeInput.substr(0, 8).toUpperCase()
const mockGroup1 = {id: 2, name: 'Group1', news: 'News1', newsChangedDate: new Date()};
const mockGroup2 = {id: 3, name: 'Group2', news: 'News2', newsChangedDate: new Date()};
const mockUser = {
    id: 1,
    name: 'User Name',
    code: 'CODE0001',
...
describe('StudentPageComponent', () => {
    ...
    test('shows data correctly from the api', async () => {
        ...
        expect(screen.getAllByRole('listitem')[0]).toHaveTextContent(mockGroup1.name);
        expect(screen.getAllByRole('listitem')[1]).toHaveTextContent(mockGroup2.name);
    ...
    test('handle join code changes correctly', async () => {
        ...
        expect(screen.getByRole('textbox'))
            .toHaveValue(mockJoinGroupCodeValue);
